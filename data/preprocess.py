...
if __name__ == '__main__':
    ...
    parser.add_argument('--frac', type=float, default=0.83333333)
    parser.add_argument('--dirichlet', type=float, default=1.0)
    parser.add_argument('--output_dir', type=str, default='split_CIFAR10_20clients_frac08333_beta_1')
    ...
    if args.dataset == 'MNIST':
        ...
        dataset = build_dataset(classes=classes, root=os.path.join(root, 'CINIC10/raw_data'))
    ...
    for i in range(args.num_clients):
        ...
        in_tot += len(in_data[i])
        out_tot += len(out_data[i])
    ...
    print('total input data: {}'.format(in_tot))
    print('total output data: {}'.format(out_tot))
