...
#         x = self.lrn2(self.pool(F.relu(self.conv2(x))))
#         x = x.view(-1, 64 * 5 * 5)
#         x = F.relu(self.fc1(x))
#         x = F.relu(self.fc2(x))
#         x = self.fc3(x)
...
def GroupNorm_(num_channels):
    return nn.GroupNorm(num_groups=2, num_channels=num_channels, eps=1e-05)
...
class BasicBlock_34(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicBlock_34, self).__init__()
        self.conv1 = conv3x3(inplanes, planes, stride)
...
def resnet34(num_classes):
    ...
    model = ResNet34(block=BasicBlock_34, 
                   layers=[3, 4, 6, 3],
